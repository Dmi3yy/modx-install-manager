    ...
    foreach ($moduleTemplates as $k=>$moduleTemplate) {
        ...
        if(in_array($k, $selTemplates) || $installSample) {
            ...
            if (!file_exists($filecontent)) {
                ...
                $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_templates` WHERE templatename='$name'");

                if (mysql_num_rows($rs)) {
                    if (!@ mysql_query("UPDATE `" . $table_prefix . "site_templates` SET content='$template', description='$desc', category=$category_id, locked='$locked'  WHERE templatename='$name';")) {
                        $errors += 1;
                    ...
                    if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_templates` (templatename,description,content,category,locked) VALUES('$name','$desc','$template',$category_id,'$locked');")) {
                        $errors += 1;
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
...
if (isset ($_POST['tv']) || $installData) {
    ...
    foreach ($moduleTVs as $k=>$moduleTV) {
        ...
        if(in_array($k, $selTVs) || $installSample) {
            ...
            $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_tmplvars` WHERE name='$name'");
            if (mysql_num_rows($rs)) {
                $insert = true;
                while($row = mysql_fetch_assoc($rs)) {
                    if (!@ mysql_query("UPDATE `" . $table_prefix . "site_tmplvars` SET type='$input_type', caption='$caption', description='$desc', category=$category, locked=$locked, elements='$input_options', display='$output_widget', display_params='$output_widget_params', default_text='$input_default' WHERE id={$row['id']};")) {
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    $insert = false;
            ...
            if (count($assignments) > 0) {
                ...
                $ds=mysql_query("SELECT id FROM `".$table_prefix."site_tmplvars` WHERE name='$name' AND description='$desc';",$sqlParser->conn);
                ...
                foreach ($assignments as $assignment) {
                    ...
                    $ts = mysql_query("SELECT id FROM `".$table_prefix."site_templates` WHERE templatename='$template';",$sqlParser->conn);
                    if ($ds && $ts) {
                        $tRow = mysql_fetch_assoc($ts);
                        $templateId = $tRow['id'];
                        mysql_query("INSERT INTO `" . $table_prefix . "site_tmplvar_templates` (tmplvarid, templateid) VALUES($id, $templateId)");
                   }
                }
            }
        }
    ...
    foreach ($moduleChunks as $k=>$moduleChunk) {
        ...
        if(in_array($k, $selChunks) || $installSample) {
            ...
            if (!file_exists($filecontent))
                ...
                $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_htmlsnippets` WHERE name='$name'");
                $count_original_name = mysql_num_rows($rs);
                if($overwrite == 'false') {
                    $newname = $name . '-' . str_replace('.', '_', $modx_version);
                    $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_htmlsnippets` WHERE name='$newname'");
                    $count_new_name = mysql_num_rows($rs);
                }
                $update = $count_original_name > 0 && $overwrite == 'true';
                if ($update) {
                    if (!@ mysql_query("UPDATE `" . $table_prefix . "site_htmlsnippets` SET snippet='$chunk', description='$desc', category=$category_id WHERE name='$name';")) {
                        $errors += 1;
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    ...
                    if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_htmlsnippets` (name,description,snippet,category) VALUES('$name','$desc','$chunk',$category_id);")) {
                        $errors += 1;
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
    ...
    foreach ($moduleModules as $k=>$moduleModule) {
        ...
        if(in_array($k, $selModules) || $installSample) {
            ...
            if (!file_exists($filecontent))
                ...
                $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_modules` WHERE name='$name'");
                if (mysql_num_rows($rs)) {
                    $row = mysql_fetch_assoc($rs);
                    $props = propUpdate($properties,mysql_real_escape_string($row['properties']));
                    if (!@ mysql_query("UPDATE `" . $table_prefix . "site_modules` SET modulecode='$module', description='$desc', properties='$props', enable_sharedparams='$shared' WHERE name='$name';")) {
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    echo "<p>&nbsp;&nbsp;$name: <span class=\"ok\">" . $_lang['upgraded'] . "</span></p>";
                } else {
                    if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_modules` (name,description,modulecode,properties,guid,enable_sharedparams,category) VALUES('$name','$desc','$module','$properties','$guid','$shared', $category);")) {
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    echo "<p>&nbsp;&nbsp;$name: <span class=\"ok\">" . $_lang['installed'] . "</span></p>";
    ...
    foreach ($modulePlugins as $k=>$modulePlugin) {
        ...
        if(in_array($k, $selPlugs) || $installSample) {
            ...
            if (!file_exists($filecontent))
                ...
                $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_plugins` WHERE name='$name'");
                if (mysql_num_rows($rs)) {
                    $insert = true;
                    while($row = mysql_fetch_assoc($rs)) {
                        $props = propUpdate($properties,mysql_real_escape_string($row['properties']));
                        if($row['description'] == $desc){
                            if (!@ mysql_query("UPDATE `" . $table_prefix . "site_plugins` SET plugincode='$plugin', description='$desc', properties='$props' WHERE id={$row['id']};")) {
                                echo "<p>" . mysql_error() . "</p>";
                                return;
                            }
                            $insert = false;
                        } else {
                            if (!@ mysql_query("UPDATE `" . $table_prefix . "site_plugins` SET disabled='1' WHERE id={$row['id']};")) {
                                echo "<p>".mysql_error()."</p>";
                                return;
                            }
                        }
                    ...
                    if($insert === true) {
                        if(!@mysql_query("INSERT INTO `".$table_prefix."site_plugins` (name,description,plugincode,properties,moduleguid,disabled,category) VALUES('$name','$desc','$plugin','$properties','$guid','0',$category);",$sqlParser->conn)) {
                            echo "<p>".mysql_error()."</p>";
                            return;
                        }
                    }
				    ...
				    //add disabled
                    if ($category == 'add'){				
						if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_plugins` (name,description,plugincode,properties,moduleguid,disabled,category) VALUES('$name','$desc','$plugin','$properties','$guid','1',$category);")) {
							echo "<p>" . mysql_error() . "</p>";
							return;
						}
					}else{	
						if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_plugins` (name,description,plugincode,properties,moduleguid,category) VALUES('$name','$desc','$plugin','$properties','$guid',$category);")) {
							echo "<p>" . mysql_error() . "</p>";
							return;
						}
					}
                ...
                // add system events
                if (count($events) > 0) {
                    $ds=mysql_query("SELECT id FROM `".$table_prefix."site_plugins` WHERE name='$name' AND description='$desc';",$sqlParser->conn);
                    if ($ds) {
                        $row = mysql_fetch_assoc($ds);
                        $id = $row["id"];
                        ...
                        // add new events
                        mysql_query("INSERT INTO `" . $table_prefix . "site_plugin_events` (pluginid, evtid) SELECT '$id' as 'pluginid',se.id as 'evtid' FROM `" . $table_prefix . "system_eventnames` se WHERE name IN ('" . implode("','", $events) . "')");
                    }
                }
            }
        }
    ...
    foreach ($moduleSnippets as $k=>$moduleSnippet) {
        ...
        if(in_array($k, $selSnips) || $installSample) {
            ...
            if (!file_exists($filecontent))
                ...
                $rs = mysql_query("SELECT * FROM `" . $table_prefix . "site_snippets` WHERE name='$name'");
                if (mysql_num_rows($rs)) {
                    $row = mysql_fetch_assoc($rs);
                    $props = propUpdate($properties,mysql_real_escape_string($row['properties']));
                    if (!@ mysql_query("UPDATE `" . $table_prefix . "site_snippets` SET snippet='$snippet', description='$desc', properties='$props' WHERE name='$name';")) {
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    echo "<p>&nbsp;&nbsp;$name: <span class=\"ok\">" . $_lang['upgraded'] . "</span></p>";
                } else {
                    if (!@ mysql_query("INSERT INTO `" . $table_prefix . "site_snippets` (name,description,snippet,properties,category) VALUES('$name','$desc','$snippet','$properties',$category);")) {
                        echo "<p>" . mysql_error() . "</p>";
                        return;
                    }
                    echo "<p>&nbsp;&nbsp;$name: <span class=\"ok\">" . $_lang['installed'] . "</span></p>";
...
function getCreateDbCategory($category, $sqlParser) {
    ...
    if(!empty($category)) {
        ...
        $rs = mysql_query("SELECT id FROM `".$table_prefix."categories` WHERE category = '".$category."'");
